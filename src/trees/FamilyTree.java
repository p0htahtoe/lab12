...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException("Colon wasn't found"); //throw a TreeException with a useful message
		...
		else
		{
			...
			if(parentNode == null) {
				throw new TreeException("Parent doesn't exist"); //If the parent node wasn't found, there must have been something wrong in the 
		...
		for(String name: childrenArray) {
			...
			parentNode.addChild(child);	// For each name in childrenArray, create a new node and add that node to parentNode.
		}
	}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null) {
			throw new TreeException("Node with first name not found");
		}		//Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null) {
			throw new TreeException("Node with second name not found");
